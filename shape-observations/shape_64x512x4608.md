# Shape Note: 64x512x4608 (Queue-C)

Last updated: 2026-03-25

## 1. Scope

Batched-ex transpose-side family member in baseline lane.

## 2. Evidence [main-node confirmed]

Sources:

- baseline split:
  - `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/g4_prefill_decode_split_gpt-oss_latest_20260325_011553.txt`
- side split:
  - `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/g4_prefill_decode_split_gpt-oss_latest_20260325_011411.txt`
- baseline full-shape compare:
  - `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/rocblas_shape_prefill_full_compare_g4_rocblas_trace_gpt-oss_latest_20260325_011553__g4_rocblas_trace_gpt-oss_latest_20260325_011629_20260325_012104.tsv`
- side full-shape compare:
  - `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/rocblas_shape_prefill_full_compare_g4_rocblas_trace_gpt-oss_latest_20260325_011411__g4_rocblas_trace_gpt-oss_latest_20260325_011439_20260325_012104.tsv`

Observed shape counts:

- baseline shape `64x512x4608`: `24 -> 24` (prefill -> full), `delta=0`
- side related transpose family:
  - `64x1024x5120`: `36 -> 36`, `delta=0`
  - `64x1024x8192`: `36 -> 36`, `delta=0`

Shared lane metrics:

- baseline: `direct/fallback/dispatch=1/1/1`, `gemm_lines=1002`, `tensile_like_rows=167`
- side: `direct/fallback/dispatch=1/1/1`, `gemm_lines=1336`, `tensile_like_rows=167`
- both lanes: `phase_split_status=prefill_dominant_signature`

## 3. Interpretation [inference]

- The transpose-side Queue-C family is stable in prefill/full proxy comparison.
- Batch-scaled side variants show higher counts while retaining the same gate class.

## 4. Related memo

- `/home/limonene/ROCm-project/ROCm-repos_AETS/rocBLAS/shape-observations/shape_64x512x4608_kernel_priority.md`
