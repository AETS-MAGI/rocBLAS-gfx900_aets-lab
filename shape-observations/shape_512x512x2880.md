# Shape Note: 512x512x2880 (Tier-1)

Last updated: 2026-03-25

## 1. Scope

Track the highest-frequency baseline shape family and its batch-scaled side lane:

- baseline: `512x512x2880`
- side: `512x1024x2880`

## 2. Evidence

### 2.1 Baseline lane (`num_batch=512`) [main-node confirmed]

Source:

- `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/g4_prefill_decode_split_gpt-oss_latest_20260325_011553.txt`

Observed:

- prefill (`num_predict=1`):
  - `direct=1`, `fallback=1`, `dispatch=1`
  - `rocblas_trace_gemm_lines=1002`
  - `kernel_tensile_like_rows=167`
  - `shape_512_512_2880=192`
- full (`num_predict=128`):
  - `direct=1`, `fallback=1`, `dispatch=1`
  - `rocblas_trace_gemm_lines=1002`
  - `kernel_tensile_like_rows=167`
  - `shape_512_512_2880=192`
- decode proxy:
  - `decode_delta(shape)=0`
  - `decode_delta_gemm_lines=0`
  - `phase_split_status=prefill_dominant_signature`

### 2.2 Side lane (`num_batch=1024`) [main-node confirmed]

Source:

- `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/g4_prefill_decode_split_gpt-oss_latest_20260325_011411.txt`

Observed:

- prefill (`num_predict=1`):
  - `direct=1`, `fallback=1`, `dispatch=1`
  - `rocblas_trace_gemm_lines=1336`
  - `kernel_tensile_like_rows=167`
  - `shape_512_1024_2880=288`
- full (`num_predict=128`):
  - `direct=1`, `fallback=1`, `dispatch=1`
  - `rocblas_trace_gemm_lines=1336`
  - `kernel_tensile_like_rows=167`
  - `shape_512_1024_2880=288`
- decode proxy:
  - `decode_delta(shape)=0`
  - `decode_delta_gemm_lines=0`
  - `phase_split_status=prefill_dominant_signature`

### 2.3 Stream phase-window cross-check (lane-level) [main-node confirmed]

Sources:

- baseline lane stream sweep:
  - `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/g4_stream_phase_window_sweep_gpt-oss_latest_20260325_022802.txt`
- side lane stream sweep:
  - `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/g4_stream_phase_window_sweep_gpt-oss_latest_20260325_022953.txt`

Observed:

- baseline (`num_batch=512`, `num_predict=64,128,256`):
  - `ok_cases=3`
  - `decode_signature_cases=3`
  - `direct/fallback/dispatch=1` for all rows
  - `decode_kernel_tensile_like_rows=167` for all rows
- side (`num_batch=1024`, `num_predict=64,128,256`):
  - `ok_cases=3`
  - `decode_signature_cases=3`
  - `direct/fallback/dispatch=1` for all rows
  - `decode_kernel_tensile_like_rows=167` for all rows

### 2.4 Refresh cycle (2026-03-25 09 JST) [main-node confirmed]

Sources:

- baseline anchor sweep:
  - `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/g4_gptoss_anchor_shape_sweep_gpt-oss_latest_20260325_090936.txt`
- side anchor sweep:
  - `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/g4_gptoss_anchor_shape_sweep_gpt-oss_latest_20260325_091016.txt`
- baseline stream sweep:
  - `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/g4_stream_phase_window_sweep_gpt-oss_latest_20260325_091108.txt`
- side stream sweep:
  - `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/g4_stream_phase_window_sweep_gpt-oss_latest_20260325_091604.txt`
- baseline split:
  - `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/g4_prefill_decode_split_gpt-oss_latest_20260325_092315.txt`
- side split:
  - `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/g4_prefill_decode_split_gpt-oss_latest_20260325_092419.txt`
- lane summary:
  - `/home/limonene/ROCm-project/vega_path_check_logs_raw/summaries/g4_anchor_lane_status_gpt-oss_latest_20260325_092620.txt`

Observed:

- baseline:
  - `shape_512_512_2880=192`
  - `direct_hits=1/1`
  - `stream decode_signature_cases=5/5`
- side:
  - `shape_512_1024_2880=288`
  - `direct_hits=1/1`
  - `stream decode_signature_cases=5/5`
- split proxy (both lanes):
  - `phase_split_status=prefill_dominant_signature`
  - `decode_delta_gemm_lines=0`
  - `decode_delta_target_shape_hits=0`

## 3. Interpretation

- [inference] This shape family is stable across prefill/full windows in both lanes.
- [inference] Increasing batch (512 -> 1024) scales frequency
  (`192 -> 288`) and gemm-line volume (`1002 -> 1336`) while keeping gate
  states and Tensile-like rows unchanged.
- [inference] In this cycle, shape-proxy split remains `prefill_dominant_signature`,
  while stream phase-window probes remain `decode_signature_detected` in both lanes.
- [inference] These two probes are complementary and should not be collapsed into one
  dispatch claim.
- [inference] 2026-03-25 09 JST refresh reproduced the same lane-level structure
  without low-level code changes.

## 4. Next checks

- Keep this family as first target for low-level correlation checks.
- If kernel-side tuning is attempted, verify this pair first:
  - `512x512x2880` (baseline)
  - `512x1024x2880` (side)
- Candidate priority memo:
  - `/home/limonene/ROCm-project/ROCm-repos_AETS/rocBLAS/shape-observations/shape_512x512x2880_kernel_priority.md`
